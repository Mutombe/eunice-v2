...
export default function Shop() {
  ...
  // Only show "empty" once the fetch has finished (otherwise the page
  // flashes empty state then content).
  if (!loading && products.length === 0) return <LoadError />

  const featured = products.find((p) => p.featured) || products[0]
  const rest = featured ? products.filter((p) => p.slug !== featured.slug) : []

  return (
    <PageTransition>
      <Seo title="Shop" path="/shop" description={shop.intro} />
      ...
      {/* Featured — full cover left, museum-label copy right.
          Only rendered once the catalogue has loaded — keeps the page
          structure intact during the brief API fetch. */}
      {featured && (
            ...
            <div className="grid md:grid-cols-12 gap-6 md:gap-12 items-stretch">
              ...
              <div className="md:col-span-5 flex flex-col justify-center md:py-4">
                ...
                <div className="mt-10 border-t border-ink/15 pt-6 flex items-end justify-between flex-wrap gap-4">
                  <div>
                    <div className="mono-sm text-ink/65">{featured.format}</div>
