...
export default function NoteCard({ note }) {
  return (
    <Link to={`/journal/${note.slug}`} className="group block">
      ...
      <div className="mt-5 grid grid-cols-12 gap-3">
        <span className="col-span-2 mono text-ink/65 tabular self-start">{note.num}</span>
        <div className="col-span-10">
          ...
          <p className="mt-3 mono-sm text-ink/65">{note.date} · {note.readTime}</p>
        </div>
      </div>
    </Link>
  )
